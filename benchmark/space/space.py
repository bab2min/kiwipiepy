...
def main(args):
    ...
    for dataset in args.datasets:
        ...
        print(os.path.basename(dataset) + " (reset spaces)", *((f'{s:.3f}' if s is not None else '-') for s in scores_ws), sep='\t')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target', default='kiwi', help='kiwi,kospacing')
    parser.add_argument('--kiwi-model-path')
    parser.add_argument('--kiwi-model-type', default='none', choices=['none', 'largest'])
    parser.add_argument('--kiwi-space-tolerance', default=2, type=int)
    parser.add_argument('--kiwi-space-penalty', default=7., type=float)
